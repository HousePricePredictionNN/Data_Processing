...
class OlxHousePricesProcessor(DataProcessor):
    ...
    def _get_paths(self, base_dir):
        ...
        input_path = get_dataset_path(base_dir, 'olx_house_prices', 'raw')
        output_path = get_dataset_path(base_dir, 'olx_house_prices', 'final')

        self.output_paths['lodz'] = get_dataset_path(base_dir, 'olx_house_prices', 'final', 'data_lodz.csv')

        return input_path, output_path

    def _read_data(self):
        """Read data from input path"""
        data_path = os.path.join(self.input_path, 'olx_house_price_Q122.csv')
