...
class OtodomCurrentPrices(DataProcessor):
    def __init__(self):
        super().__init__('otodom_current_prices')

    def _get_paths(self, base_dir):
        """Define input/output paths"""
        input_path = get_dataset_path(base_dir, 'otodom_current_prices', 'raw')
        output_path = get_dataset_path(base_dir, 'otodom_current_prices', 'final')

        return input_path, output_path

    def _read_data(self):
        """Read data from input path"""
        data_path = os.path.join(self.input_path, 'otodom_data.csv')
